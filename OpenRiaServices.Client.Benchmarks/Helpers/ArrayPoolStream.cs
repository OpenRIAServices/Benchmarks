...
namespace OpenRiaServices.Hosting.AspNetCore.Serialization
{
    ...
    internal sealed class ArrayPoolStream : Stream
    {
        ...
        /// Copies bytes from <paramref name="src"/> to <paramref name="dest"/>
        /// </summary>
        private static unsafe void FastCopy(ReadOnlySpan<byte> src, byte[] dest, int destOffset, int count)
        {
            Unsafe.CopyBlockUnaligned(destination: ref dest[destOffset], source: ref MemoryMarshal.GetReference(src), (uint)count);
        }
        ...
        public struct BufferMemory : IDisposable
        {
            ...
            public async Task WriteTo(HttpResponse response, CancellationToken ct)
            {
                ...
                await WriteToAsync(response.BodyWriter);
            ...
            public async Task WriteTo2(HttpResponse response, CancellationToken ct)
            {
                await response.StartAsync(ct);
                await WriteTo2(response.BodyWriter);
            ...
            {
                await response.StartAsync(ct);
                await WriteToAsync(response.Body);
            ...
            private async Task WriteToAsync(Stream stream)
            {
                ...
                if (list != null)
                {
                    ...
                    {
                        await stream.WriteAsync(list[i]);
                    }
                }

                if (_buffer != null)
                {
                    await stream.WriteAsync(_buffer.AsMemory(0, _bufferWritten));
                }
            }

            private async Task WriteToAsync(PipeWriter bodyWriter)
            {
                ...
                if (_buffer != null)
                {
                    Write(bodyWriter, _buffer, _bufferWritten);
                    await bodyWriter.FlushAsync();
                }
            }

            private static void Write(PipeWriter bodyWriter, byte[] buffer, int len)
            {
            ...
            private async Task WriteTo2(PipeWriter bodyWriter)
            {
                var list = _bufferList;
                if (list != null)
                {
                    for (int i = 0; i < list.Count; ++i)
                    {
                        await bodyWriter.WriteAsync(list[i]);
                    }
                }

                if (_buffer != null)
                {
                    await bodyWriter.WriteAsync(_buffer.AsMemory(0, _bufferWritten));
                }
            }
